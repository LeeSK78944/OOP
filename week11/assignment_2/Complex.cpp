...
Number *Complex::add(Number * num) {
	...
	if (num->types() == INTEGER) {
         Integer *nI = dynamic_cast<Integer *>(num);
         this->set_val(this->real()+(double)nI->val(), this->imag());
         delete(nI);
         return this;
     } else if (num->types() == FLOAT) {
         Float *nF = dynamic_cast<Float *>(num);
         this->set_val(this->real()+nF->val(), this->imag());
		 delete(nF);
		 return this;
     } else if (num->types() == COMPLEX) {
         Complex *nC = dynamic_cast<Complex *>(num);
         this->set_val(this->real()+nC->real(), this->imag()+nC->imag());
		 delete(nC);
		 return this;
     } else {
         //empty
...
Number *Complex::mul(Number * num) {
	...
	if (num->types() == INTEGER) {
          Integer *nI = dynamic_cast<Integer *>(num);
          this->set_val(this->real()*(double)nI->val(), (double)nI->val()*this->imag());
          delete(nI);
          return this;
      } else if (num->types() == FLOAT) {
          Float *nF = dynamic_cast<Float *>(num);
          this->set_val(this->real()*nF->val(), nF->val()*this->imag());
          delete(nF);
          return this;
      } else if (num->types() == COMPLEX) {
          Complex *nC = dynamic_cast<Complex *>(num);
          this->set_val((this->real()*nC->real())-(nC->imag()*this->imag()), (nC->imag()*this->real())+(this->imag()*nC->real()));
          delete(nC);
          return this;
      } else {
          //empty
...
Number *Complex::div(Number * num) {
	...
	if (num->types() == INTEGER) {
          ...
          Complex *nC = dynamic_cast<Complex *>(num);
          this->set_val(
				  (this->real() * nC->real() + this->imag() * nC->imag()) / (nC->real() * nC->real() + nC->imag() * nC->imag()),
            (this->imag() * nC->real() - (this->real() * nC->imag())) / (nC->real() * nC->real() + nC->imag() * nC->imag()));
          delete(nC);
          return this;
      } else {
          //empty
